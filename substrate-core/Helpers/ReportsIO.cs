...
namespace substrate_core.Helpers
{
    public static class ReportsIO
    {
        public static IResilienceReport Build(IResilienceTracker tracker, InventoryManager inventoryManager)
        {
            IReportBuilder builder = new ReportBuilder(tracker, inventoryManager);
        ...
        public static void Print(IResilienceReport report)
        {
            Console.WriteLine("=== Resilience Report ===");
            Console.WriteLine($"Duels: {report.DuelCount} | Total Resilience: {report.TotalResilience:F2} | Resilience Index: {report.ResilienceIndex:F2}");
            Console.WriteLine($"Outcomes → Recoveries: {report.RecoveryCount}, Collapses: {report.CollapseCount}, Wounds: {report.WoundCount}, Conflicts: {report.ConflictCount}, Equilibriums: {report.EquilibriumCount}");
            Console.WriteLine($"Narrative Summary: {report.GenerateNarrative()}");
        }
        public static void Print(ISummary summary)
        {
            Console.WriteLine("=== Duel Summary ===");
            // Assuming ISummary exposes properties like Outcome, Bias, Tone, etc.
            Console.WriteLine(summary.ToString()); 
            // Or expand with explicit fields if ISummary has them
        }
