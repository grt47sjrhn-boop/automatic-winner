...
namespace substrate_core.Models.Engagements.Types
{
    public class DuelEngagement : IEngagement
    {
        ...
        public IRarityTier Rarity { get; private set; } = new RarityTier("Common", "Default rarity tier.");

        public FacetDistribution Shape { get; private set; } = new();
        public BiasDescriptor Bias { get; private set; } = new();
        public IToneCut Brilliance { get; private set; }
...
// Add duel tension to cumulative resilience for reporting
        ...
        public ISummary Finalize()
        {
            ...
            var title = "Duel Engagement Complete";
            var description = $"Resilience index: {delta} (Cumulative: {_cumulativeResilience})";

            // 🔹 Outcome decision based on bias value
            var outcome = Bias.Value switch
            {
            ...
            var rarityEnum = Rarity.Tier switch
            {
        ...
        private static FacetDistribution MergeFacetDistributions(FacetDistribution a, FacetDistribution b)
        {
            ...
            {
                var va = a.Values.GetValueOrDefault(type, 0);
                var vb = b.Values.GetValueOrDefault(type, 0);
                result[type] = va + vb;
            }
        ...
        private static Dictionary<ToneType, int> SafeToneDictionary(FacetDistribution facets)
        {
            ...
            int get(FacetType t) => values.GetValueOrDefault(t, 0);
